...
M4_StabSelection <- function(DATA, Jk, R, LassoSequence, N_loading, Thr, NRSTARTS, N_cores){
  ...
  result <- RegularizedSCA::cv_sparseSCA(DATA, Jk, R, MaxIter = 400, NRSTARTS = 1, LassoSequence, GLassoSequence=0, nfolds = 7, method = "component")  
  ...
  sim_result <- foreach::foreach(i = 1:100, .combine='+') %dorng% {
    ...
    result <- RegularizedSCA::sparseSCA(DATA, Jk, R, LASSO = LassoSequence[1], GROUPLASSO = 0, MaxIter = 400, NRSTARTS = 5, method = "component")
  ...
  for(j in 2:length(LassoSequence)){
    cl <- snow::makeCluster(N_cores)
    doSNOW::registerDoSNOW(cl)
    #note that set.seed() and %dorng% ensure that parallel computing generates reproducable results.
    sim_result <- foreach::foreach(i = 1:100, .combine='+') %dorng% {
  ...
  thr <- sort(P_final, decreasing = TRUE)[N_loading] # this is the lowest probability whose corresponding loading should not be zero.
