...
result <- matrix(NA, length(LassoSequence), length(GLassoSequence))
for (i in 1:length(LassoSequence)){
  for (j in 1:length(GLassoSequence)){
    
    result[i,j] <- BIC_SCA(DATA=Xgenerate, Jk, R, LassoSequence[i], GLassoSequence[j], MaxIter=MAXITER, NRSTARTS)$BIC
  }
}

OptimalLasso <- LassoSequence[which(result == min(result), arr.ind=T)[1]]
OptimalGLasso <- GLassoSequence[which(result == min(result), arr.ind=T)[2]]
...
for (n in 1:NRSTARTS){
  if(method == "datablock"){
    VarSelectResultOpt <- CDfriedmanV1(DATA=Xgenerate, Jk, R, OptimalLasso, OptimalGLasso, MaxIter = MAXITER)
  }else if (method == "component"){
    VarSelectResultOpt <- CDfriedmanV2(DATA=Xgenerate, Jk, R, OptimalLasso, OptimalGLasso, MaxIter = MAXITER)
  }
